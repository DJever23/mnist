...
def imageprepare():
    """
        ...
        输出：灰度图像像素值
    """
    file_name='/home/dengjie/dengjie/project/Mnist/mnist/mnist_test/6a.png'#导入自己的图片地址
    #图片格式转换
    im = Image.open(file_name)
    plt.imshow(im)  #显示需要识别的图片
    plt.show()
    im = im.convert('L')#'L'模式图片
    #im.save("/home/dengjie/dengjie/project/Mnist/mnist/sample/sample.png")#保存经过格式转换后的图片
...
sess.run(tf.global_variables_initializer())
    # 这里使用了之前保存的模型参数
#saver.restore(sess, "/home/dengjie/dengjie/project/Mnist/mnist/deepmodel/model.ckpt")
